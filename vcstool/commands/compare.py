...
class CompareCommand(Command):
    ...
    def __init__(self, args: argparse.Namespace) -> None:
        args.path = self._remove_trailing_slashes(args.path)
        super().__init__(args)
        self.path = args.path
    ...
    def execute(self) -> List[Dict[str, Any]]:
        ...
        clients = find_repositories([self.path], nested=self.nested)
    ...
    def _remove_trailing_slashes(path: str) -> str:
        """Removes trailing slash from a path and returns it."""
        # Allow root directory search if desired.
        if path == "/":
            return path
        while path.endswith("/"):
            path = path[:-1]
...
def read_repos_from_manifest_file(manifest_file: str) -> Dict[str, Dict[str, Any]]:
    if manifest_file == "":
        print_err(
            "No manifest file specified. Please specify one using the --input flag."
        )
        return None
...
def main(args=None, stdout=None, stderr=None) -> None:
    ...
    args = CompareCommand.get_parser().parse_args(args)
